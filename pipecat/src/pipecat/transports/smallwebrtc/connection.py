...
class SmallWebRTCConnection(BaseObject):
    ...
    def force_transceivers_to_send_recv(self):
        ...
        # Dynamically choose direction based on whether a sender track is attached.
        # If no outbound track is present, keep transceiver as recvonly to avoid encoder tasks.
        for i, transceiver in enumerate(transceivers):
            has_sender_track = bool(getattr(transceiver, "sender", None) and getattr(transceiver.sender, "track", None))
            if i < 2 and has_sender_track:
    ...
    def replace_audio_track(self, track):
        ...
        logger.debug(f"Replacing audio track {kind if kind else 'None'}")
        ...
        if len(transceivers) > 0 and transceivers[0].sender:
            if track:
                ...
                transceivers[0].sender.replaceTrack(None)
    ...
    def replace_video_track(self, track):
        ...
        """
        kind = getattr(track, "kind", None)
        logger.debug(f"Replacing video track {kind if kind else 'None'}")
        # Transceivers always appear in creation-order for both peers
        # For now we are only considering that we are going to have 02 transceivers,
        # one for audio and one for video
        transceivers = self._pc.getTransceivers()
        if len(transceivers) > 1 and transceivers[1].sender:
            if track:
                ...
                # Explicitly detach any existing track and set to recvonly to avoid encoder startup
                transceivers[1].sender.replaceTrack(None)
